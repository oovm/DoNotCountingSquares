...
img = Import@"Task.PNG";
{w, h} = {260,160}
img=Thinning@ColorNegate@Binarize[ImageResize[ImageCrop@img,{w,h}]];
...
Show[ListLinePlot[Join[col, row], PlotStyle -> {Gray}, AspectRatio ->  h/w ], Graphics[{PointSize[0.01], Red, Point[corners]}]]
...
getSVG[a_ -> b_] := Block[
	...
	Export[
		"Size-" <> ToString[a / 400] <> ".SVG",
		Multicolumn[Flatten[Graphics[g[#]]& /@ b], {Automatic, 4}, Appearance -> "Horizontal"]
	]
];
getSVG /@ grouped
...
![](./Size-`1`.SVG)
\n"];
tail = StringJoin[str @@@ ({#1 / 400, Length@#2}& @@@ grouped)];
